...
function TopBar({ productMenu, utilityMenu }: TopBarProps) {
  ...
  return (
    <div
      className="flex h-full w-screen flex-col text-sm md:h-9 md:w-full md:flex-row md:items-center md:justify-center lg:items-center lg:gap-10"
    ...
    >
      <nav className="flex h-full w-full flex-col gap-5 pt-5 pb-5 md:ml-52 md:flex-row md:items-center md:justify-start md:gap-0 md:pt-0 md:pb-0">
        {productMenu.map((item, index) => (
          <li
            key={item.name}
            className={`bl-1 flex h-full w-full list-none items-center pr-8 pl-8 text-center font-medium hover:font-bold md:border-l md:border-gray-300 ${index == productMenu.length - 1 ? "md:border-r" : ""} ${activeTabIndex == index ? "md:border-t-2 md:border-orange-500 md:bg-white md:font-bold" : "md:font-extralight"} `}
            onClick={() => setActiveTabIndex(index)}
          >
            <a
              className="flex items-center gap-1 whitespace-nowrap lg:block lg:gap-0"
              href={item.link}
            >
